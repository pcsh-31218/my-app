...
function App() {
  ...
  const getTextContent = () => messages.map(m => {
  ...
  return (
    <div className="app-root">
      ...
      <div className="role-switcher">
        ...
        <button className={`role-tab ${activeRole === "interviewer" ? "active" : ""}`} onClick={() => setActiveRole("interviewer")}>{interviewerName || "面試官"}</button>
        <button className={`role-tab ${activeRole === "interviewee" ? "active" : ""}`} onClick={() => setActiveRole("interviewee")}>{intervieweeName || "應試者"}</button>
        <span className="tab-hint">Tab 切換</span>
      </div>

      <div className="chat-area">
        {messages.length === 0 && <div className="chat-empty">✦ 開始你的訪談對話 ✦</div>}
        {messages.map((msg, i) => {
          ...
          return (
            <div key={msg.id} className={`msg-row ${isMe ? "right" : "left"}`}>
              ...
              <div className="msg-body">
                ...
                <div className={`bubble ${isMe ? "bubble-right" : "bubble-left"}`}>{msg.content}</div>
      ...
      <div className="input-bar">
        <textarea ref={textareaRef} value={value} onChange={e => setValue(e.target.value)} onKeyDown={handleKeyDown} rows={1} placeholder={`以「${myName}」身分輸入…`} className="chat-input" />
